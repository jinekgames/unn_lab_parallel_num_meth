...
template<Number T>
concept class = std::is_same_v<T, float> || std::is_same_v<T, double>;
...
void SmartMPCycle(size_t start, size_t end, const std::function<void(size_t)>& op, size_t TargetItPerThrd = 200) {
    ...
    static const size_t threadsCount   =
        threadsCountSystem *
        static_cast<size_t>(std::fminl(1, static_cast<double>(numIterations) /
                                       TargetItPerThrd));
    ...
    for (size_t tid = 0; tid < threadsCount; ++tid) {
        ...
        size_t newEnd   = std::min(newStart + itersPerThread, end);
